...
const ImageRemoteComponent = (props: ImageRemoteProps) => {
  const { style: styleOverride = {}, imgSource, styleDefault = {}, resizeMode = 'contain', containerStyle, ...rest } = props;
  const [url, setUrl] = React.useState('')
  const style: ImageStyle = React.useMemo(() => enhance([ROOT, styleOverride]), []);
  const styleImgDefault: ImageStyle = React.useMemo(() => enhance([defaultStyle, styleDefault]), []);
  React.useEffect(() => {
