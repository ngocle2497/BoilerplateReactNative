    ...
    const _renderItem = ({ item, index }: { item: MessageProps; index: number }) => {
        return (
            <MessageItem
                ...
                nextDateCreate={data.length - 1 <= index ? null : data[index + 1].dateCreate + ""}
                prevType={index === 0 ? null : data[index - 1].sourceMessage} />
        )
    }
    const _keyExtractor = (item: MessageProps, index: number) => item.id + "";
    ...
    return (
        <Block block color={'#FFFFFF'}>
            {renderHeader ? renderHeader() : <Block style={styles.top} direction={'row'}>
                ...
                <Block width={SIZE_AVATAR * 2} height={SIZE_AVATAR * 2} borderRadius={SIZE_AVATAR}>
                    <ImageRemote style={styles.imgAvatar} styleDefault={styles.imgAvatar} imgSource={friendAvatar} />
                    <Block style={[styles.dotStatus]}
                        borderWidth={scale(1)} borderColor={'#ffffff'}
            ...
            <Block style={styles.bottom} direction={'row'}>
                <Block block style={[styles.wrapInput]}>
                    <TextInput value={value} onChangeText={_onChangText} placeholder={'Aa'} underlineColorAndroid={'transparent'} selectionColor={'#333333'} style={[styles.input]} />
                </Block>
                <Button onPress={_onSendPress} style={[styles.buttonSend]} preset={'link'}>
...
export const Chat = memo(ChatComponent, (prevProps, nextProps) => isEqual(prevProps, nextProps))
