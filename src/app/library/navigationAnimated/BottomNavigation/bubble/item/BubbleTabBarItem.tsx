...
import { useValues, withTransition, interpolateColor, onGestureEvent } from 'react-native-redash'
import { DEFAULT_ITEM_INNER_SPACE, DEFAULT_ITEM_OUTER_SPACE } from '../constant'
import Animated, { add, cond, eq, interpolate, useCode, set } from 'react-native-reanimated'
import { State, createNativeWrapper, TouchableWithoutFeedback } from 'react-native-gesture-handler'

const AnimatedRawButton = createNativeWrapper(Animated.createAnimatedComponent(TouchableWithoutFeedback), {
    shouldActivateOnStart: false,
    shouldCancelWhenOutside: true,
});

const gestureHandler = (state: Animated.Value<State>) => onGestureEvent({ state })
...
const BubbleTabBarItemComponent = (props: TabBarItemProps) => {
    ...
    const { itemInnerHorizontalSpace,
        itemInnerVerticalSpace,
        itemOuterHorizontalSpace,
        itemOuterVerticalSpace } = useMemo(() => {
            ...
            if (typeof itemInnerSpace === 'number') {
                ...
                _itemInnerVerticalSpace = itemInnerSpace?.vertical || DEFAULT_ITEM_INNER_SPACE;
                _itemInnerHorizontalSpace = itemInnerSpace?.horizontal || DEFAULT_ITEM_INNER_SPACE;
            }
            ...
            if (typeof itemOuterSpace === 'number') {
                ...
                _itemOuterVerticalSpace = itemOuterSpace?.vertical || DEFAULT_ITEM_OUTER_SPACE;
                _itemOuterHorizontalSpace = itemOuterSpace?.horizontal || DEFAULT_ITEM_OUTER_SPACE;
            }
            return {
    ...
    const minWidth = useMemo(() => {
        return (
            itemInnerVerticalSpace * 2 + iconSize + itemInnerHorizontalSpace * 2
        )
    ...
    const colorTintIcon = cond(eq(selectedIndex, index), icon.activeColor, icon.inactiveColor)
    ...
    const contentContainerStyle = [
        styles.contentContainer,
        {
            ...
            backgroundColor: interpolateColor(animatedFocused, {
                inputRange: [0, 1],
    ...
    const renderIcon = () => {
        return typeof icon.component === 'function'
            ? icon.component({ color: animatedIconFocused, size: iconSize, colorTintIcon: colorTintIcon })
            : icon.component;
    }
    useCode(() => cond(eq(state, State.END), [
...
const BubbleTabBarItem = memo(BubbleTabBarItemComponent, (prevProps, nextProps) => equals(prevProps, nextProps))
