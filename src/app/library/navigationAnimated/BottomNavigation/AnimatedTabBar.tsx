...
import { TabBarViewProps, TabBarItemConfigurableProps, TabBarAnimationConfigurableProps, TabsConfigsType } from './type'
...
interface AnimatedTabBarProps extends Pick<BottomTabBarProps, 'state' | 'descriptors' | 'navigation'>,
...
export const AnimatedTabBar = (props: AnimatedTabBarProps) => {
    ...
    const { routes, index: navigationIndex, key: navigationKey }: { routes: Route<string>[]; index: number; key: string; } = useMemo(() => {
        return props.state;
    }, [props])
    const [selectedIndex] = useValues([0], [])

    const getRouteTitle = useCallback((route: Route<string>) => {
    ...
    const getRouteTabConfigs = useCallback((route: Route<string>) => {
    ...
    useCode(() => onChange(selectedIndex, call([selectedIndex], args => {
