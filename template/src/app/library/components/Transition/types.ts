...
export type TransitionAnimationProp<Animate> =
  TransitionProps<Animate>['animate'];
export type TransitionFromProp<Animate> = TransitionProps<Animate>['from'];

export type TransitionTransitionProp<
  Animate = StyleValueWithReplacedTransforms<
...
> {
   ...
   * ```
   */
  transition?: TransitionTransitionProp<AnimateWithTransitions>;
...
export type Variants<
  V,
  // these generics are copied from 'TransitionProps'
  // Style props of the component
  // defaults to any styles, so that generics aren't Required
  // in component usage, it will extract these from the style prop ideally
  AnimateType = ImageStyle & TextStyle & ViewStyle,
